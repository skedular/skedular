...
public class StripePaymentMethodConfiguration : IEntityTypeConfiguration<StripePaymentMethod>
{
    public void Configure(EntityTypeBuilder<StripePaymentMethod> builder)
    {
        ...
        builder.Property(item => item.ClientSecret).HasMaxLength(Constants.MaxStripeClientSecretLength);
        builder
            .Property(item => item.Status)
            .HasMaxLength(Constants.MaxStripePaymentMethodStatusLength)
            .HasDefaultValue(StripePaymentMethodStatusConstants.Pending);
