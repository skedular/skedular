...
- Event protobufs live under `api-definitions/events/skedular/*.proto`. Changing them requires running
  `api-definitions/events/generate.sh`. That now:
    - builds `src/shared/Api.Shared.Clients/Api.Shared.Clients.csproj`, which regenerates protobuf event C# classes into
      `src/shared/Api.Shared.Clients/obj`

- OpenAPI contracts live under `api-definitions/openapi/skedular/*.yaml`. Changing them requires running
  `api-definitions/openapi/generate.sh`, which regenerates:
    - C# service controller bases under `src/shared/Api.Shared.Services/OpenApi`
    - C# API clients under `src/shared/Api.Shared.Clients/OpenApi`

- GraphQL schema export is driven by `scripts/generate-graphql.sh`. It regenerates:
...
- Web OpenAPI TypeScript clients are generated by `src/web/apps/webapp/scripts/generate.sh`. If an OpenAPI YAML changes
  and the web app consumes that API, rerun this script.
...
## Relay Mutation Update Rule

- A successful GraphQL mutation in a Relay UI MUST NOT call `window.location.reload()` to show server state.
- Request the fields that rendered components need from the mutation payload, including the stable `id`, so Relay can
  normalize the changed record into its store.
- When a mutation also changes a connection, aggregate, or linked records that its payload does not return, use the
  appropriate declarative connection update or a targeted Relay query refetch. Do not use a browser reload as a cache
  invalidation mechanism.
- A browser reload is permitted only for an explicit, user-initiated recovery action in the shared error boundary; it is
  not an acceptable mutation-success handler.
- After changing a Relay operation, run `pnpm --dir src/web relay` and commit the generated artifacts.

- gRPC protobufs live under `api-definitions/grpc/skedular/**/*.proto`. Their C# code is generated by the consuming
  `.csproj` files during build, so do not hand-edit generated gRPC outputs.

## Event Generation Shape

- **Shared event contracts** (`IEvent`, `IEventExtensions`, `KafkaTopicAttribute`, `KafkaTopicHelper`,
  `EventMetadataFactory`) are owned by `src/shared/Api.Shared/` (netstandard2.0 for cross-platform compatibility).
...
- Those event protobuf classes are generated into `src/shared/Api.Shared.Clients/obj` at build time.
- The checked-in files under `src/shared/Api.Shared.Clients/Events/Skedular/**` are **handwritten metadata companions
...
- Keep versioned event metadata under the matching version directory, for example
  `src/shared/Api.Shared.Clients/Events/Skedular/Booking/V1/BookingMetadata.cs`.
- See `src/shared/Api.Shared/AGENTS.md` and `src/shared/Api.Shared.Clients/AGENTS.md` for the complete event
  architecture.
...
- It runs, in order:
    ...
    3. `src/web/apps/webapp/scripts/generate.sh`
...
- **Api.Shared Portability Rule**: `src/shared/Api.Shared` targets **netstandard2.0** for cross-platform compatibility.
  Do not add framework-specific dependencies or APIs that are unavailable in netstandard2.0. When updating event
  contracts in `Api.Shared`, validate that downstream projects (`Api.Shared.Clients`,
  `Enterprise.Shared`, domains) still compile and test successfully.
- For user-facing or operator-facing English copy, use American spelling and grammar rather than British English.
- Apply that localization to UI text, validation messages, alerts, notifications, emails, and exception messages that
  surface to users or operators, but do not rename code identifiers, API fields, schema names, routes, or other
  technical contracts just for localization.
...
- API and transport layers—including GraphQL resolvers, REST controllers, gRPC handlers, and transport mappers—must not
  inject `IRepositoryFactory` or repository implementations at all. They may call services only; move repository reads
  and writes behind service methods that return shared models, then map those models to transport types.
...
- Reusable Xero configuration and Enterprise-level service registration live in
  `src/shared/Enterprise.Shared/Accounting`.
...
- For Xero repeating invoice webhooks, do not assume the webhook `resourceId` matches the stored local repeating link.
  ...
  for generated recurring invoices carry the concrete generated invoice id. Resolve the concrete invoice from Xero, read
...
- Future work: recurring payment reconciliation needs cadence-level allocation, not only template-level paid state. For
  split recurring cadences such as a quarterly pass billed monthly, booking must eventually record which generated
...
- If no matching auto-renewable pricing exists anymore, the subscription must move to renewal failed instead of silently
  renewing on the wrong pricing.
...
- In `src/shared/Enterprise.Shared`, keep the auth/encryption concerns split by responsibility:
...
- In `src/web/apps/webapp`, do not import `@mui/material/Typography` directly in feature/page components.
- Use the existing Skedular typography wrappers exported from `@/components/commons` instead, for example
  `BodyIconTypography`, `SmallIconTypography`, `CaptionIconTypography`, `LeadIconTypography`, and the heading wrappers.
- The only exception is inside the shared typography wrapper implementation files themselves under
  `src/web/apps/webapp/src/components/commons`, where direct MUI `Typography` is allowed as the low-level primitive.
...
- For cache wrappers, when a repository call returns no data and the service catches `*NotFound`, log the outcome as
  "lookup returned no result" or "not found" instead of "cache miss" unless you are instrumenting cache layer hits and
  misses explicitly.
...
## Unit-test and nullability conventions

- Prefer `[Theory]` with `AutoFakeItEasyData` for unit tests. Inject dependencies, the SUT, and inputs through
  parameters in that order.
- Avoid in-test `A.Fake<T>()` construction when auto-data can provide the dependency; configure only required calls with
  `A.CallTo`.
- Required services and infrastructure collaborators are never nullable: loggers, transaction builders, repository
  factories, mappers, clocks, and domain services must be supplied by DI.
- Never add `?`, `?.`, null-forgiving operators, null fallback behavior, or null constructor arguments merely to support
  tests. Optional collaborators require an explicit domain contract.

For additional context about technologies to be used, project structure, shell commands, and other important
information, read the current plan at specs/037-refund-reliability/plan.md
...
- C# .NET 10 backend; TypeScript 6.0.3, React 19.2.6, Next.js 16.2.6 App Router frontend + HotChocolate/Fusion GraphQL,
  REST/OpenAPI definitions where needed, EF Core/PostgreSQL, Temporal workflows, Kafka/protobuf organization events,
  Relay 21, `react-relay`, MUI 9, `@skedular/ui`, `@skedular/shared`, existing public-web data/rendering modules
  (027-teams-pricing-catalogue)
- PostgreSQL via Organization-owned EF Core persistence for subscription/catalog state; existing domain projections in
  Team, Location, and Booking continue consuming public organization state/events (027-teams-pricing-catalogue)

- TypeScript 6.0.3; React 19.2.6; Next.js 16.2.6 App Router; backend C# .NET 10 only if GraphQL/domain contract changes
  are needed + Relay 21, `react-relay`, MUI 9, `@skedular/ui`, `@skedular/shared`, WorkOS AuthKit, Leaflet/react-leaflet
  for map browsing, existing marketplace GraphQL schema and generated Relay artifacts (020-customer-landing-cleanup)
- No new persistence planned for the first cleanup/design slice; uses existing marketplace booking, subscription,
  location, organization, and customer data via GraphQL. Any new durable cleanup inventory can start as feature
  documentation/task artifact unless implementation requires product-owned persistence. (020-customer-landing-cleanup)

- C# .NET 10 for customer/booking/organization/team/marketplace/location APIs, processors, shared libraries, and
  integration tests; protobuf event definitions under `api-definitions/events/skedular` + Existing Kafka protobuf event
  envelope and metadata companions, Enterprise.Shared Kafka producer/consumer/outbox infrastructure, EF Core
  repositories and migrations, customer workaround republish API, HotChocolate/Fusion GraphQL/auth integration points
  where readiness gating is enforced, Microsoft/Serilog structured logging conventions (016-customer-readiness-tracking)
- PostgreSQL via EF Core in `src/customer/shared/Customer.Shared`; add central readiness state tables/entities and
  repository methods. Participating domains reuse existing local customer/identity persistence.
  (016-customer-readiness-tracking)
...
- C# .NET 10 for organization API/domain services; GraphQL schema generated by HotChocolate/Fusion tooling;
...
- PostgreSQL via EF Core; no new migration planned because display name and description already map to existing
  organization fields/metadata (010-organization-patch-updates)
