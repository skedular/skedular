...
- Event protobufs live under `api-definitions/events/skedular/*.proto`.
  Changing them requires running `api-definitions/events/generate.sh`, which regenerates C# event code under
...
- Both can reuse the same low-level encryption algorithm, but they must stay separate at the service and configuration boundary.
