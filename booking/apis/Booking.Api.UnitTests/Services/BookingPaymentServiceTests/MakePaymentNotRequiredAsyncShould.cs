...
public class MakePaymentNotRequiredAsyncShould
{
    [Theory]
    [AutoFakeItEasyData]
    public async Task Update_The_Marketplace_Booking_Without_Releasing_Resources(
        ...
        CancellationToken cancellationToken)
    {
        ...
        marketplaceBooking.PaymentStatus.ShouldBe(PaymentStatus.NoPaymentRequired.ToPaymentStatus());
        A.CallTo(() => bookingResourceSlotsHelperService.RemoveAllSlotsFromBooking(A<BookingEntity>._)).MustNotHaveHappened();
        A.CallTo(() => temporalOutboxService.SignalWorkflowPayBookingViaBankTransferSetPaymentStatus(
                bookingId,
                A<SetPaymentStatusArgs>.That.Matches(item => item.PaymentStatus == PaymentStatus.NoPaymentRequired.ToPaymentStatus()),
                unitOfWork))
