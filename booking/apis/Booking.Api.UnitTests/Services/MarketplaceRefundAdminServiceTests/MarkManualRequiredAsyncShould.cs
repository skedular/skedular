...
public class MarkManualRequiredAsyncShould
{
    [Theory]
    [AutoFakeItEasyData]
    public async Task Update_Refund_Status_To_Manual_Required_And_Raise_Booking_Graphql_Change(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var result = await sut.MarkManualRequiredAsync(refund.Id, "Bank payout handled offline", cancellationToken);

        result.ShouldBe(refund);
        refund.Status.ShouldBe(MarketplaceRefundStatusConstants.ManualRequired);
        refund.Reason.ShouldBe("Bank payout handled offline");
