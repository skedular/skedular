...
public record PayRecurringBookingViaCardInput(string RecurringBookingId, DateTimeOffset ExpiryDate);
        ...
        try
        {
            ...
            var upsertBookingRelatedStripeCustomerResponse = await Workflow.ExecuteActivityAsync(
                (StripeIntegrations activity) => activity.UpsertRecurringBookingRelatedStripeCustomerAsync(
                    new UpsertRecurringBookingRelatedStripeCustomerInput(args.RecurringBookingId, upsertProductAndPricingResponse.StripeConnectAccountId)),
                new ActivityOptions
                {
                    StartToCloseTimeout = TimeSpan.FromSeconds(30),
                    TaskQueue = Workflow.Info.TaskQueue,
