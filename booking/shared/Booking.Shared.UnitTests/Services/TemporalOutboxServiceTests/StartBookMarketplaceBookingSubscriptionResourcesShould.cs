...
public class StartBookMarketplaceBookingSubscriptionResourcesShould
{
    [Theory]
    [AutoFakeItEasyData]
    public void Enqueue_Workflow_With_Correct_Options(
        ...
        string expectedId)
    {
        A.CallTo(() => workflowIdService.BookMarketplaceBookingSubscriptionResources(args.MarketplaceBookingSubscriptionId))
            .Returns(expectedId);

        sut.StartBookMarketplaceBookingSubscriptionResources(args, unitOfWork);

        A.CallTo(() => temporalOutboxWorkflowExecutor
            .Execute<BookMarketplaceBookingSubscriptionResources, BookMarketplaceBookingSubscriptionResourcesInput>(
                args,
