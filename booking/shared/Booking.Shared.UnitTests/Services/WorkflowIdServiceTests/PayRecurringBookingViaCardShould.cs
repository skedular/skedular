...
public class PayRecurringBookingViaCardShould
{
    [Theory]
    [AutoFakeItEasyData]
    ...
    {
        var rawWorkflowId = $"{Constants.PaidRecurringBookingViaCardPrefix}-{recurringBookingId}";

        A.CallTo(() => temporalHelperService.ToId(rawWorkflowId)).Returns(expectedWorkflowId);

        sut.PayRecurringBookingViaCard(recurringBookingId).ShouldBe(expectedWorkflowId);
    }
}
