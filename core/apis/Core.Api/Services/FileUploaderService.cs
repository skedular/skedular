...
{
    Task<(string, Uri)> UploadAsync(Stream stream, string contentType, CancellationToken cancellationToken);
}
...
{
    public async Task<(string, Uri)> UploadAsync(Stream stream, string contentType, CancellationToken cancellationToken)
    {
        var (_, customerEntity) = await customerService.GetCustomerAsync(cancellationToken);

        var id = randomHelper.Generate();
        var (storageUrl, cdnUrl) = await cdnService.UploadAsync(stream, contentType, id, cancellationToken);
