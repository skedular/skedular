...
resource "vercel_project" "default" {
  ...
  build_command    = "pnpm webapp-help#build"
  install_command  = "pnpm install --recursive --frozen-lockfile"
  output_directory = "./apps/webapp-help/.next"
...
resource "vercel_project_domain" "default" {
  ...
  team_id    = local.team_id
  domain     = module.shared_common.help_webapp_domain_name
}
...
resource "cloudflare_dns_record" "default" {
  ...
  name    = module.shared_common.help_webapp_domain_name
