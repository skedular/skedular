...
const MyPaymentMethodSetupForm = ({ onCancel }: Props) => {
  ...
  const handleAddClick = async () => {
    ...
    const { error } = await stripe.confirmSetup({
      elements,
      confirmParams: {
        return_url: `${process.env.NEXT_PUBLIC_API_ENDPOINT}/v1/customer/add-payment-method`,
      },
    });
...
export default memo(MyPaymentMethodSetupForm);
