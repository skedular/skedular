...
import {
    getOrganizationBankAccountBaseLink,
    getOrganizationBaseLink,
    getOrganizationProductBaseLink,
...
import {
    ExistingStripeConnectAccountButton,
    NewStripeConnectAccountButton
} from '@/components/stripeConnectAccount/addStripeConnectAccount';
...
import type {
    organizationMarketplaceSetup_activateProductsMutation
} from '@/queries/__generated__/organizationMarketplaceSetup_activateProductsMutation.graphql';
import type {
    organizationMarketplaceSetup_deactivateProductsMutation
} from '@/queries/__generated__/organizationMarketplaceSetup_deactivateProductsMutation.graphql';
import type {
    organizationMarketplaceSetup_deleteLocationTagsMutation
} from '@/queries/__generated__/organizationMarketplaceSetup_deleteLocationTagsMutation.graphql';
import type {
    organizationMarketplaceSetup_deleteOrganizationBankAccountsMutation
} from '@/queries/__generated__/organizationMarketplaceSetup_deleteOrganizationBankAccountsMutation.graphql';
import type {
    organizationMarketplaceSetup_deleteOrganizationStripeConnectAccountsMutation
} from '@/queries/__generated__/organizationMarketplaceSetup_deleteOrganizationStripeConnectAccountsMutation.graphql';
import type {
    organizationMarketplaceSetup_deleteProductsMutation
} from '@/queries/__generated__/organizationMarketplaceSetup_deleteProductsMutation.graphql';
import type {
    organizationMarketplaceSetup_deleteProductTagsMutation
} from '@/queries/__generated__/organizationMarketplaceSetup_deleteProductTagsMutation.graphql';
import type {
    organizationMarketplaceSetup_locationTags_query$key
} from '@/queries/__generated__/organizationMarketplaceSetup_locationTags_query.graphql';
import type {
    organizationMarketplaceSetup_locationTags_refetchableFragment
} from '@/queries/__generated__/organizationMarketplaceSetup_locationTags_refetchableFragment.graphql';
import type {
    organizationMarketplaceSetup_organizationBankAccounts_query$key
} from '@/queries/__generated__/organizationMarketplaceSetup_organizationBankAccounts_query.graphql';
import type {
    organizationMarketplaceSetup_organizationBankAccounts_refetchableFragment
} from '@/queries/__generated__/organizationMarketplaceSetup_organizationBankAccounts_refetchableFragment.graphql';
import type {
    organizationMarketplaceSetup_organizationStripeConnectAccounts_query$key
} from '@/queries/__generated__/organizationMarketplaceSetup_organizationStripeConnectAccounts_query.graphql';
import type {
    organizationMarketplaceSetup_organizationStripeConnectAccounts_refetchableFragment
} from '@/queries/__generated__/organizationMarketplaceSetup_organizationStripeConnectAccounts_refetchableFragment.graphql';
import type {
    organizationMarketplaceSetup_products_query$key
} from '@/queries/__generated__/organizationMarketplaceSetup_products_query.graphql';
import type {
    organizationMarketplaceSetup_products_refetchableFragment
} from '@/queries/__generated__/organizationMarketplaceSetup_products_refetchableFragment.graphql';
import type {
    organizationMarketplaceSetup_productTags_query$key
} from '@/queries/__generated__/organizationMarketplaceSetup_productTags_query.graphql';
import type {
    organizationMarketplaceSetup_productTags_refetchableFragment
} from '@/queries/__generated__/organizationMarketplaceSetup_productTags_refetchableFragment.graphql';
import type {
    organizationMarketplaceSetup_query$key
} from '@/queries/__generated__/organizationMarketplaceSetup_query.graphql';
import type {
    organizationMarketplaceSetup_setOrganizationBankAccountAsDefaultMutation
} from '@/queries/__generated__/organizationMarketplaceSetup_setOrganizationBankAccountAsDefaultMutation.graphql';
import type {
    organizationMarketplaceSetup_setOrganizationStripeConnectAccountAsDefaultMutation
} from '@/queries/__generated__/organizationMarketplaceSetup_setOrganizationStripeConnectAccountAsDefaultMutation.graphql';
...
import OrganizationMarketplaceSetupLeftSideNavigationMenuContent
    from './organization-marketplace-setup-left-side-navigation-menu-content';
...
const OrganizationMarketplaceSetup = ({
                ...
                if (errors && errors.length > 0) {
                    toast.update(toastId, {
                        ...
                        render: <NotificationContent
                            content={`Failed to remove product. Error: ${joinErrors(errors)}.`}/>,
                    });
    ...
    const handleDeactivateProductsClick = () => {
        const toastId = themedToast(<NotificationContent
            content={'Deactivating products...'}/>, infoNotificationOptions);
            ...
            onCompleted: (_, errors) => {
                if (errors && errors.length > 0) {
                    toast.update(toastId, {
                        ...errorNotificationOptions,
                        render: <NotificationContent
                            content={`Failed to deactivate products. Error: ${joinErrors(errors)}`}/>,
                    });
            ...
            onCompleted: (_, errors) => {
                if (errors && errors.length > 0) {
                    toast.update(toastId, {
                        ...errorNotificationOptions,
                        render: <NotificationContent
                            content={`Failed to activate products. Error: ${joinErrors(errors)}`}/>,
                    });
    ...
    const handleDeactivateProductClick = () => {
        ...
        const toastId = themedToast(<NotificationContent
            content={'Deactivating product...'}/>, infoNotificationOptions);
            ...
            onCompleted: (_, errors) => {
                if (errors && errors.length > 0) {
                    toast.update(toastId, {
                        ...errorNotificationOptions,
                        render: <NotificationContent
                            content={`Failed to deactivate product. Error: ${joinErrors(errors)}`}/>,
                    });
            ...
            onCompleted: (_, errors) => {
                if (errors && errors.length > 0) {
                    toast.update(toastId, {
                        ...errorNotificationOptions,
                        render: <NotificationContent
                            content={`Failed to activate product. Error: ${joinErrors(errors)}`}/>,
                    });
    ...
    const handleRemoveProductTagsClick = () => {
        const toastId = themedToast(<NotificationContent
            content="Removing product tags ..."/>, infoNotificationOptions);
            ...
            onCompleted: (_, errors) => {
                if (errors && errors.length > 0) {
                    toast.update(toastId, {
                        ...errorNotificationOptions,
                        render: <NotificationContent
                            content={`Failed to remove product tags. Error: ${joinErrors(errors)}.`}/>,
                    });
            ...
            onCompleted: (_, errors) => {
                if (errors && errors.length > 0) {
                    toast.update(toastId, {
                        ...errorNotificationOptions,
                        render: <NotificationContent
                            content={`Failed to remove product tag. Error: ${joinErrors(errors)}.`}/>,
                    });
    ...
    const handleRemoveLocationTagsClick = () => {
        const toastId = themedToast(<NotificationContent
            content="Removing location tags ..."/>, infoNotificationOptions);
            ...
            onCompleted: (_, errors) => {
                if (errors && errors.length > 0) {
                    toast.update(toastId, {
                        ...errorNotificationOptions,
                        render: <NotificationContent
                            content={`Failed to remove location tags. Error: ${joinErrors(errors)}.`}/>,
                    });
    ...
    const handleRemoveLocationTagClick = () => {
        ...
        const toastId = themedToast(<NotificationContent
            content="Removing location tag ..."/>, infoNotificationOptions);
            ...
            onCompleted: (_, errors) => {
                if (errors && errors.length > 0) {
                    toast.update(toastId, {
                        ...errorNotificationOptions,
                        render: <NotificationContent
                            content={`Failed to remove location tag. Error: ${joinErrors(errors)}.`}/>,
                    });
    ...
    const handleSetOrganizationStripeConnectAccountAsDefault = () => {
        ...
        const toastId = themedToast(
            <NotificationContent
                content={`Setting stripe connect account ${organizationStripeConnectAccountDetails.name} as default...`}/>,
            infoNotificationOptions,
        );
        ...
        commitSetOrganizationStripeConnectAccountAsDefault({
            ...
            onCompleted: (_, errors) => {
                if (errors && errors.length > 0) {
                    toast.update(toastId, {
                        ...
                        render: (
                            <NotificationContent
                                content={`Failed to set stripe connect account ${organizationStripeConnectAccountDetails.name} as default. Error: ${joinErrors(errors)}`}/>
                        ),
                    });
                ...
                toast.update(toastId, {
                    ...successNotificationOptions,
                    render: <NotificationContent
                        content={`Stripe connect account ${organizationStripeConnectAccountDetails.name} is set as default.`}/>,
                });
            ...
            onError: (error) => {
                toast.update(toastId, {
                    ...errorNotificationOptions,
                    render: <NotificationContent
                        content={`Failed to set stripe connect account ${organizationStripeConnectAccountDetails.name} as default. Error: ${error.message}.`}/>,
                });
            },
            optimisticResponse: {
    ...
    const handleRemoveOrganizationStripeConnectAccountsClick = () => {
        const toastId = themedToast(<NotificationContent
            content="Removing Stripe Connect accounts ..."/>, infoNotificationOptions);
        ...
        commitDeleteOrganizationStripeConnectAccounts({
            ...
            onCompleted: (_, errors) => {
                if (errors && errors.length > 0) {
                    toast.update(toastId, {
                        ...errorNotificationOptions,
                        render: <NotificationContent
                            content={`Failed to remove Stripe Connect accounts. Error: ${joinErrors(errors)}.`}/>,
                    });
            ...
            onError: (error) => {
                toast.update(toastId, {
                    ...errorNotificationOptions,
                    render: <NotificationContent
                        content={`Failed to remove Stripe Connect accounts. Error: ${error.message}.`}/>,
                });
            },
        });
    };
    ...
    const handleRemoveOrganizationStripeConnectAccountClick = () => {
        ...
        const toastId = themedToast(<NotificationContent
            content="Removing Stripe Connect account ..."/>, infoNotificationOptions);
        ...
        commitDeleteOrganizationStripeConnectAccounts({
            ...
            onCompleted: (_, errors) => {
                if (errors && errors.length > 0) {
                    toast.update(toastId, {
                        ...errorNotificationOptions,
                        render: <NotificationContent
                            content={`Failed to remove Stripe Connect account. Error: ${joinErrors(errors)}.`}/>,
                    });
            ...
            onError: (error) => {
                toast.update(toastId, {
                    ...errorNotificationOptions,
                    render: <NotificationContent
                        content={`Failed to remove Stripe Connect account. Error: ${error.message}.`}/>,
                });
            },
        });
    };
    ...
    const handleSetOrganizationBankAccountAsDefault = () => {
        ...
        const toastId = themedToast(<NotificationContent
            content={`Setting bank account ${organizationBankAccountDetails.name} as default...`}/>, infoNotificationOptions);
        ...
        commitSetOrganizationBankAccountAsDefault({
            ...
            onCompleted: (_, errors) => {
                if (errors && errors.length > 0) {
                    toast.update(toastId, {
                        ...errorNotificationOptions,
                        render: <NotificationContent
                            content={`Failed to set bank account ${organizationBankAccountDetails.name} as default. Error: ${joinErrors(errors)}`}/>,
                    });
                ...
                toast.update(toastId, {
                    ...successNotificationOptions,
                    render: <NotificationContent
                        content={`Bank account ${organizationBankAccountDetails.name} is set as default.`}/>,
                });
            ...
            onError: (error) => {
                toast.update(toastId, {
                    ...errorNotificationOptions,
                    render: <NotificationContent
                        content={`Failed to set bank account ${organizationBankAccountDetails.name} as default. Error: ${error.message}.`}/>,
                });
            },
            optimisticResponse: {
    ...
    const handleRemoveOrganizationBankAccountsClick = () => {
        const toastId = themedToast(<NotificationContent
            content="Removing Bank accounts ..."/>, infoNotificationOptions);
            ...
            onCompleted: (_, errors) => {
                if (errors && errors.length > 0) {
                    toast.update(toastId, {
                        ...errorNotificationOptions,
                        render: <NotificationContent
                            content={`Failed to remove Bank accounts. Error: ${joinErrors(errors)}.`}/>,
                    });
    ...
    const productColumns: GridColDef<(typeof productRows)[number]>[] = [
        ...
        },
        {
            ...
            editable: false,
            renderCell: (params) => <SmallIconTypography
                label={params.value ? `${params.value} minutes` : 'No limit'}/>,
            display: 'flex',
            minWidth: 120,
        },
        {
    ...
    const organizationStripeConnectAccountColumns: GridColDef<(typeof organizationStripeConnectAccountRows)[number]>[] = [
        ...
        },
        {
            ...
            renderCell: (params) => {
                ...
                return <CompleteOnboardStripeConnectAccountButton onboardingUrl={account.onboardingUrl}
                    ...
                    <AppBarWithStackColumn onClose={handleCloseClick} label="Edit Marketplace Information">
                            ...
                            <StackRow sx={{paddingLeft: defaultPadding, paddingRight: defaultPadding}}>
                                ...
                                >
                                    <StackRow sx={{alignItems: 'center'}}>
                                        <SmallIconTypography
                                            label={`${seledctedOrganizationStripeConnectAccounts.ids.size} records selected`}/>
                                        <PushToRight/>
                                        <Button
                                            size="medium"
                                            variant="contained"
                        ...
                        {seledctedOrganizationBankAccounts.ids.size > 0 && (
                            <StackRow sx={{paddingLeft: defaultPadding, paddingRight: defaultPadding}}>
                                ...
                                >
                                    <StackRow sx={{alignItems: 'center'}}>
                                        <SmallIconTypography
                                            label={`${seledctedOrganizationBankAccounts.ids.size} records selected`}/>
                                        <PushToRight/>
                                        <Button
                                            size="medium"
                                            variant="contained"
                        ...
                        >
                            <GridContainer sx={{justifyContent: 'space-between'}}>
                                ...
                                <Grid>
                                    <AddOrganizationProductTagButton organizationId={organizationId}
                                                                     connectionIds={productTagsConnectionIds}/>
                                </Grid>
                            </GridContainer>
                            <Divider/>
                        ...
                        <GridContainer spacing={1} sx={{padding: defaultPadding}}>
                            <PushToRight/>
                            <Search size="small" placeholder="Search for product tags"
                                    defaultValue={productTagNameSearchText}
                                    onChange={handleProductTagsSearchTextChange}/>
                        </GridContainer>
                        {seledctedProductTags.ids.size > 0 && (
                            <StackRow sx={{paddingLeft: defaultPadding, paddingRight: defaultPadding}}>
                                ...
                                >
                                    <StackRow sx={{alignItems: 'center'}}>
                                        <SmallIconTypography
                                            label={`${seledctedProductTags.ids.size} records selected`}/>
                                        <PushToRight/>
                                        <Button size="medium" variant="contained" color="warning"
                                                startIcon={<DeleteIcon/>} onClick={handleRemoveProductTagsClick}
                                                sx={{textTransform: 'none'}}>
                                            Remove Product Tag
                                        </Button>
                                    </StackRow>
                        ...
                        >
                            <GridContainer sx={{justifyContent: 'space-between'}}>
                                ...
                                <Grid>
                                    <AddOrganizationLocationTagButton organizationId={organizationId}
                                                                      connectionIds={locationTagsConnectionIds}/>
                                </Grid>
                            </GridContainer>
                            <Divider/>
                        ...
                        <GridContainer spacing={1} sx={{padding: defaultPadding}}>
                            <PushToRight/>
                            <Search size="small" placeholder="Search for location tags"
                                    defaultValue={locationTagNameSearchText}
                                    onChange={handleLocationTagsSearchTextChange}/>
                        </GridContainer>
                        {seledctedLocationTags.ids.size > 0 && (
                            <StackRow sx={{paddingLeft: defaultPadding, paddingRight: defaultPadding}}>
                                ...
                                >
                                    <StackRow sx={{alignItems: 'center'}}>
                                        <SmallIconTypography
                                            label={`${seledctedLocationTags.ids.size} records selected`}/>
                                        <PushToRight/>
                                        <Button size="medium" variant="contained" color="warning"
                                                startIcon={<DeleteIcon/>} onClick={handleRemoveLocationTagsClick}
                                                sx={{textTransform: 'none'}}>
                                            Remove Location Tag
                                        </Button>
                                    </StackRow>
                        ...
                        <GridContainer spacing={1} sx={{padding: defaultPadding}}>
                            <PushToRight/>
                            <Search size="small" placeholder="Search for product" defaultValue={productNameSearchText}
                                    onChange={handleProductsSearchTextChange}/>
                        </GridContainer>
                                ...
                                >
                                    <StackRow sx={{alignItems: 'center'}}>
                                        ...
                                        <Button size="medium" variant="contained" color="secondary"
                                                onClick={handleDeactivateProductsClick} sx={defaultButtonStyle}>
                                            Deactivate Product
                                        </Button>
                                        <Button size="medium" variant="contained" color="secondary"
                                                onClick={handleActivateProductsClick} sx={defaultButtonStyle}>
                                            Activate Product
                                        </Button>
                                        <Button size="medium" variant="contained" color="warning"
                                                startIcon={<DeleteIcon/>} onClick={handleRemoveProductsClick}
                                                sx={{textTransform: 'none'}}>
                                            Remove Product
                                        </Button>
                                    </StackRow>
