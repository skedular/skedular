import { getOrganizationLocationAddLink } from '@/components/links';
...
const NewLocationButton = ({ organizationId, fullWidth, label, hideIcon, variant, size }: Props) => (
  <Button href={getOrganizationLocationAddLink(organizationId)} variant={variant ?? 'text'} fullWidth={fullWidth}>
    {size === 'small' && <SmallIconTypography label={label ?? 'Add Location'} endElement={hideIcon ? null : <NewIcon fontSize={size ?? 'small'} />} />}
    {size === 'medium' && <BodyIconTypography label={label ?? 'Add Locationm'} endElement={hideIcon ? null : <NewIcon fontSize={size ?? 'medium'} />} />}
