...
import type {
    BookingType,
    bookProduct_addBookingMutation,
    PaymentMethod
} from '@/queries/__generated__/bookProduct_addBookingMutation.graphql';
import type {
    bookProduct_availableResources_query$key
} from '@/queries/__generated__/bookProduct_availableResources_query.graphql';
import type {
    bookProduct_availableResources_refetchableFragment
} from '@/queries/__generated__/bookProduct_availableResources_refetchableFragment.graphql';
...
const BookProduct = ({
    ...
    const resources = useMemo<ResourceDetails[]>(
        () =>
            timeRangeValid
                ? rootDataAvailableResources.availableResources.map(({
                                                                         uniqueId,
                                                                         name,
                                                                         location,
                                                                         customTags,
    ...
    const handleAddClick = ({
        ...
        const toastId = themedToast(<NotificationContent
            content={`Making a booking on '${fromToPrint}'...`}/>, infoNotificationOptions);
        ...
        commitAddBooking({
                ...
                if (errors && errors.length > 0) {
                    toast.update(toastId, {
                        ...
                        render: <NotificationContent
                            content={`Failed to make a booking '${fromToPrint}'. Error: ${joinErrors(errors)}.`}/>,
                    });
            ...
            onError: (error) => {
                toast.update(toastId, {
                    ...errorNotificationOptions,
                    render: <NotificationContent
                        content={`Failed to make a booking '${fromToPrint}'. Error: ${error.message}.`}/>,
                });
            },
            optimisticResponse: {
                addBooking: {
                                ...
                                <FormStackColumn onSubmit={handleSubmit}>
                                    ...
                                    <StackColumn sx={{
                                        ...
                                        <FormFieldLabel label="Date/Time">
                                            <StackColumn>
                                                <StackRow>
                                                    ...
                                                    <TimeRangePicker minutesStep={rootData.openingHoursMinutesStep}
                                                                     defaultValue={timeRange} onChange={setTimeRange}/>
                                                </StackRow>
                                            </StackColumn>
                                        </FormFieldLabel>
                                        ...
                                        <FormFieldLabel label="Notes">
                                            <TextField name="notes" required={requiredFields.notes}
                                        ...
                                        <FormFieldLabel label="Type">
                                            <SingleChoiceMarketplaceBookingType rootDataRelay={rootData} name="type"
                                            ...
                                            <StackColumn>
                                                ...
                                                >
                                                    ...
                                                    {locations.map((item) => (
                                                        <MenuItem key={item.uniqueId} value={item.uniqueId}>
                                                            <BodyIconTypography
                                                                startElement={<LocationAvatar name={{name: item.name}}
                                                                                              size="small"/>}
                                                                label={item.name}/>
                                                        </MenuItem>
                                                    ))}
                                                ...
                                                <StackRow>
                                                    <DefaultSelect
                                                        ...
                                                        renderValue={(selectedId) => {
                                                            ...
                                                            if (selectedItem) {
                                                                return (
                                                                    <StackRow>
                                                                        <LeadIconTypography label="Tag" startElement={
                                                                            <CustomTagIcon/>}/>
                                                                        <Divider orientation="vertical" flexItem/>
                                                                        <PushToRight/>
                                                                        <SmallIconTypography label={selectedItem.name}/>
                                                    ...
                                                    >
                                                        ...
                                                        {customTags.map((item) => (
                                                            <MenuItem key={item.id} value={item.id}>
                                                                <BodyIconTypography startElement={<CustomTagIcon/>}
                                                                                    label={item.name}/>
                                                            </MenuItem>
                                                        ))}
                                                    </DefaultSelect>
                                                    ...
                                                    <DefaultSelect
                                                        ...
                                                        renderValue={(selectedId) => {
                                                            ...
                                                            if (selectedItem) {
                                                                return (
                                                                    <StackRow>
                                                                        <LeadIconTypography label="Zone"
                                                                                            startElement={<ZoneIcon/>}/>
                                                                        <Divider orientation="vertical" flexItem/>
                                                                        <PushToRight/>
                                                                        <SmallIconTypography label={selectedItem.name}/>
                                                                    </StackRow>
                                                    ...
                                                    >
                                                        ...
                                                        {zones.map((item) => (
                                                            <MenuItem key={item.id} value={item.id}>
                                                                <BodyIconTypography startElement={<ZoneIcon/>}
                                                                                    label={item.name}/>
                                                            </MenuItem>
                                                        ))}
                                                    </DefaultSelect>
                                        ...
                                        <FormFieldLabel label="Resources">
                                            {filteredResources.length > 0 && quantity > 0 && (
                                                <BodyIconTypography
                                                    label={`Feel free to choose up to ${product.numberOfResourcesToBook * quantity} resources from the list below!`}/>
                                            )}
                                            ...
                                            {filteredResources.length === 0 && <BodyIconTypography
                                                label="There are currently no available resources."/>}
                                        </FormFieldLabel>
                                        ...
                                        {sendInvoice && (
                                            <FormFieldLabel label="">
                                                <MultipleChoicesUserEmails rootDataRelay={rootData}
                                                                           name="invoiceEmailList"
                                                                           required={requiredFields.invoiceEmailList}/>
                                            </FormFieldLabel>
                                        )}
