...
- `location/` is still allowed to keep replicated organization, organization-member, customer, and customer-identity state when that state is needed for local authorization and access checks.
- Do not remove auth-critical replicas just because they originate in another domain.
- Only booking-derived and other non-auth passive replicas are candidates for replacement with workflow-driven background fetches.

## Agent Rule

- Availability and slot generation logic tends to have time-boundary edge cases. Check generated side effects when editing this domain.
