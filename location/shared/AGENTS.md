...
- `location/shared/` may still keep replicated organization, organization-member, customer, and customer-identity state
  when that state is required for local authorization.
- Those auth-critical replicas are separate from booking-derived analytics and should not be removed by applying the
  analytics pattern too broadly.
