...
public class GenerateLocationDailyAnalyticsShould
{
    [Theory]
    [AutoFakeItEasyData]
    ...
    {
        var rawWorkflowId = $"{Constants.GenerateLocationDailyAnalyticsPrefix}-{locationId}";

        A.CallTo(() => temporalHelperService.ToId(rawWorkflowId)).Returns(expectedWorkflowId);

        sut.GenerateLocationDailyAnalytics(locationId).ShouldBe(expectedWorkflowId);
    }
}
