...
public class CustomerRepository(MsTeamsDbContext dbContext, TimeProvider timeProvider)
    : RepositoryBase<MsTeamsDbContext, Customer>(dbContext), ICustomerRepository
{
    ...
    private static readonly Func<MsTeamsDbContext, CancellationToken, Task<ICollection<Customer>>>
        s_getAllQueryAsync =
            EF.CompileAsyncQuery<MsTeamsDbContext, CancellationToken, ICollection<Customer>>((
                    dbContext,
                    cancellationToken) =>
                dbContext.Customer
                    .AddDependentObjects()
