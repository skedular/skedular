...
public interface IAzureTenantTeamChannelRepository : IRepository<AzureTenantTeamChannel>
{
    ...
    void RemoveRange(IEnumerable<AzureTenantTeamChannel> tenantMembers);
}
        ...
        IAzureTenantTeamChannelRepository
{
    ...
    public void RemoveRange(IEnumerable<AzureTenantTeamChannel> tenantMembers)
    {
        var now = TimeProvider.GetUtcNow();
        DbContext.AzureTenantTeamChannel.UpdateRange(tenantMembers.Select(item =>
        {
            item.DeletedAt = now;
            return item;
        }));
