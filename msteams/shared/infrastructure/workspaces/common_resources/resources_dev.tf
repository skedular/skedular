resource "azuread_application" "msteams_dev" {
  ...
  web {
    redirect_uris = [
      "https://localhost:15002/auth-end.html"
    ]
  }
...
resource "azuread_application_identifier_uri" "msteams_identifier_uris_dev" {
  count          = var.environment == "staging" ? 1 : 0
  application_id = azuread_application.msteams_dev[count.index].id
  identifier_uri = "api://localhost:15002/${azuread_application.msteams_dev[count.index].client_id}"
}
...
resource "aws_ssm_parameter" "msteams_dev" {
  count = var.environment == "staging" ? 1 : 0
  name  = module.common.parameter_store_name_azure_msteams_dev_application_id
  type  = "String"
  value = azuread_application.msteams_dev[count.index].client_id
  tags  = local.tags
}

resource "azuread_application_password" "msteams_dev" {
  count          = var.environment == "staging" ? 1 : 0
  application_id = azuread_application.msteams_dev[count.index].id
}

resource "aws_ssm_parameter" "msteams_dev_secret_id" {
  count = var.environment == "staging" ? 1 : 0
  name  = module.common.parameter_store_name_azure_msteams_dev_secret_id
  type  = "String"
  value = azuread_application_password.msteams_dev[count.index].key_id
  tags  = local.tags
}

resource "aws_ssm_parameter" "msteams_dev_secret_value" {
  count = var.environment == "staging" ? 1 : 0
  name  = module.common.parameter_store_name_azure_msteams_dev_secret_value
  type  = "String"
  value = azuread_application_password.msteams_dev[count.index].value
  tags  = local.tags
}
