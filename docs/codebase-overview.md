...
| Project | Purpose |
|---|---|
| `Api.Shared.Clients` | Typed clients and protobuf event metadata for inter-service communication |
...
Services publish and consume domain events over Kafka. Events are defined as protobuf schemas under `api-definitions/events/skedular/`. Each domain's `processors/` project contains the Kafka consumer/processor logic.
