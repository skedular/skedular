...
public class NotificationService(IRepositoryFactory repositoryFactory, ICachedCustomerService cachedCustomerService, IMapper mapper)
    : INotificationService
{
    public async Task<int> PendingInvitationsCountAsync(CancellationToken cancellationToken)
    {
        var (customer, _) = await cachedCustomerService.GetAsync(cancellationToken);
        return await repositoryFactory.NotificationRepository.PendingInvitationsCountAsync(customer.Id, cancellationToken);
    }

    public async Task<(PaginatedInfo, ICollection<Edge<Shared.Models.Notification>>, int)> GetMyPaginatedNotificationsAsync(
        PaginationInputParam paginationInputParam,
        NotificationSearchCriteria searchCriteria,
        ICollection<NotificationOrder> orderByFields,
        CancellationToken cancellationToken)
    {
        var (customer, _) = await cachedCustomerService.GetAsync(cancellationToken);
        // Ensure we do not return another customer notification by forcing CustomerId as search criteria
