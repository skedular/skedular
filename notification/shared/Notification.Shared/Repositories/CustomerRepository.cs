...
public class CustomerRepository(NotificationDbContext dbContext, TimeProvider timeProvider)
    : RepositoryBase<NotificationDbContext, Customer>(dbContext), ICustomerRepository
{
    private static readonly Func<NotificationDbContext, string, CancellationToken, Task<Customer?>>
        s_getByIdQueryAsync =
            EF.CompileAsyncQuery<NotificationDbContext, string, CancellationToken, Customer?>((
                    dbContext,
                    id,
                    cancellationToken) =>
                dbContext.Customer
    ...
    private static readonly Func<NotificationDbContext, CancellationToken, Task<ICollection<Customer>>>
        s_getAllQueryAsync =
            EF.CompileAsyncQuery<NotificationDbContext, CancellationToken, ICollection<Customer>>((
                    dbContext,
                    cancellationToken) =>
                dbContext.Customer
                    .AddDependentObjects()
