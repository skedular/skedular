...
public class InvokeAsyncShould
{
    [Theory]
    [AutoFakeItEasyData]
    public async Task Call_next_when_no_bearer_token(IContext context, string token)
    {
        ...
        A.CallTo(() => tokenService.VerifyTokenAsync(A<string>._, A<CancellationToken>._))
            .MustNotHaveHappened();
    }

    [Theory]
    [AutoFakeItEasyData]
    public async Task Verify_token_when_bearer_present(IContext context, string token)
    {
        var httpContext = new DefaultHttpContext();
        ...
        nextCalled.ShouldBeTrue();
        A.CallTo(() => tokenService.VerifyTokenAsync(token, A<CancellationToken>._))
