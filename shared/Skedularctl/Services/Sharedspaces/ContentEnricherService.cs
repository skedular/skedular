...
{
    ICollection<string> ExtractEmails(string content);
    ICollection<string> ExtractWebsites(string content);
}

public class ContentEnricherService : IContentEnricherService
{
    public ICollection<string> ExtractEmails(string content)
    {
    ...
    public ICollection<string> ExtractWebsites(string content)
    {
        var urlRegex = new Regex(
            @"\b((https?|ftp):\/\/[^\s/$.?#].[^\s]*)\b",
            RegexOptions.IgnoreCase);

        var wwwRegex = new Regex(
            @"\b(www\.[^\s/$.?#].[^\s]*)\b",
            RegexOptions.IgnoreCase);

        var bareDomainRegex = new Regex(
            @"\b((?:[a-z0-9-]+\.)+[a-z]{2,})(?:/[^\s]*)?\b",
            RegexOptions.IgnoreCase);
