...
{
    private readonly IEnumerable<GeneratableTypeVisitor> _generatableTypeExtractors;

    /// <param name="generatableTypeExtractors">The visitors to use for lookup.</param>
    public GeneratableTypeProvider(params GeneratableTypeVisitor[] generatableTypeExtractors)
        : this(generatableTypeExtractors as IEnumerable<GeneratableTypeVisitor>)
    {
    }

    /// <param name="generatableTypeExtractors">The visitors to use for lookup.</param>
    public GeneratableTypeProvider(
        IEnumerable<GeneratableTypeVisitor> generatableTypeExtractors) =>
    ...
    public ImmutableHashSet<IGeneratableType> FromSchema(ISchema schema)
    {
        ...
        var field = typeof(Schema).GetField("_additionalInstances", BindingFlags.NonPublic | BindingFlags.Instance);
        if (field is null)
        {
    ...
    public async Task<ImmutableHashSet<IGeneratableType>> FromSchemaFilePathAsync(string schemaFilePath)
    {
        if (File.Exists(schemaFilePath) is not true)
        {
            throw new FileNotFoundException("No schema with provided path was found.",
                schemaFilePath);
        }
    ...
    public ImmutableHashSet<IGeneratableType> FromSchemaText(string schemaText)
    {
        if (string.IsNullOrWhiteSpace(schemaText))
        {
            throw new ArgumentNullException(nameof(schemaText),
                "Schema text could not be null or empty.");
        }
