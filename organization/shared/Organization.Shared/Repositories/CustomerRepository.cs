...
public class CustomerRepository(OrganizationDbContext dbContext, TimeProvider timeProvider)
    : RepositoryBase<OrganizationDbContext, Customer>(dbContext), ICustomerRepository
{
    ...
    private static readonly Func<OrganizationDbContext, CancellationToken, Task<ICollection<Customer>>>
        s_getAllQueryAsync =
            EF.CompileAsyncQuery<OrganizationDbContext, CancellationToken, ICollection<Customer>>((
                    dbContext,
                    cancellationToken) =>
                dbContext.Customer
                    .AddDependentObjects()
