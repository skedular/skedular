...
- **Cleanup reconciliation.** `MarketplaceBookingCleanupReconciliationService` now queries durable failures, atomically claims candidates with a lease, records attempts, and enqueues the idempotent `MarketplaceBookingCleanup` workflow from the daily reconciliation host. It covers failures without payment records. Immediate enqueue directly from retry exhaustion remains a follow-up hardening item; the durable candidate is still repaired by the scheduled pass.
