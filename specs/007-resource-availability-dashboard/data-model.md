...
One booking time window within a resource's day view. Detail fields are redacted (nulled) for non-admin users in Marketplace/Individual organizations.
...
public sealed record ResourceAvailabilityDayFilter
{
    ...
    public string? OrganizationId                                      { get; init; }  // tenancy scope
}
...
2. Backend computes a deterministic `subscriptionKey` from the canonicalised filter fields (`locationId`, `floorId`, `zoneId`, `resourceType`, `organizationId`, `date`) — e.g. a URL-safe base64 of a SHA-256 hash of the canonical JSON.
...
**No-filter state**: when all filter fields are null (initial load, all resources shown), the server generates a key encoding the organization-scoped empty filter. Booking events fire to this key on every mutation for the tenant, so empty-filter subscribers receive updates too.
...
public static class SubscriptionKeyService
{
    public static string Compute(ResourceAvailabilityDayFilter filter)
    {
        ...
        {
            org   = filter.OrganizationId ?? string.Empty,
        ...
        DateOnly date)
    {
        ...
        // combined with the fixed (organizationId, locationId, date).
...
foreach (var key in SubscriptionKeyService.AffectedKeys(
    booking.OrganizationId, booking.LocationId,
...
| `LocationId` (if provided) must belong to the requesting user's accessible organizations              | `ResourceAvailabilityDayViewService` tenancy check |
