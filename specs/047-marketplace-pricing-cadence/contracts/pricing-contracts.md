# Pricing Contracts: Marketplace pricing membership term Simplification

## Shared and event contract

The marketplace pricing value contains `membershipTerm` and does not contain `bookingCadence`. The event enum contains `NOT_SET` plus the ten supported offer terms. Removed enum values are absent from the source contract and regenerated outputs.
...
- `ProductPricing` exposes `membershipTerm` only; `bookingCadence` is removed.
- `MembershipTerm` choice/detail queries expose only the supported values, with `NOT_SET` available where the existing choice contract permits cadence-free entitlements.
...
- Marketplace event mapping maps only membership term.
- Booking projection and shared services consume membership term for term/renewal behavior and organization billing cycle for internal slices.
