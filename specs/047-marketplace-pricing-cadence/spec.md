# Feature Specification: Marketplace pricing membership term Simplification

**Feature Branch**: `047-marketplace-pricing-cadence`
**Created**: 2026-08-31
**Status**: Draft
**Input**: User description: "Redesign marketplace pricing so `MembershipTerm` is the only cadence field. Remove `BookingCadence` entirely, remove sub-day cadence values, make booking duration depend only on min/max duration, and make credit entitlements cadence-free."
...
4. **Given** a longer membership term, **When** billing or resource booking slices are created, **Then** the organization billing cycle may determine those slices without changing the selected membership term.
...
**Why this priority**: Booking duration is a customer-facing behavior and must be predictable independently of the commercial membership term.
...
### User Story 3 - membership term-free credit entitlements (Priority: P1)

As a customer purchasing credits, I want the entitlement to be defined by credits and validity rules rather than by a recurring membership term.
...
- A renewing offer must renew by its membership term, while generated invoice and resource-booking slices may follow the organization billing cycle.
...
- **FR-001**: The marketplace pricing model MUST use `MembershipTerm` as its only cadence field; `BookingCadence` MUST be removed from pricing data, persistence, serialization, contracts, projections, services, workflows, frontend models, and tests.
- **FR-002**: The supported `MembershipTerm` values MUST be exactly Daily, Weekly, Fortnightly, Monthly, TwoMonths, Quarterly, FourMonths, FiveMonths, SixMonths, and Yearly.
...
- **FR-004**: `MembershipTerm` MUST represent the marketplace offer or contract term and MUST NOT be renamed in this feature.
- **FR-005**: Auto-renewal MUST determine whether the membership term repeats. A non-renewing offer MUST create one term only; a renewing offer MUST renew using its `MembershipTerm`.
- **FR-006**: Organization billing cycle MUST continue to control invoice and resource-booking slices inside longer membership terms and MUST NOT replace or reinterpret `MembershipTerm`.
...
- **ProductPricing**: A marketplace offer containing price, membership term, renewal setting, entitlement attributes, and booking duration limits.
- **MembershipTerm**: The finite set of supported offer/contract terms of one day or longer.
- **Marketplace subscription**: A renewable purchase whose repeated terms are controlled by the offer’s membership term and auto-renewal setting.
...
- **Organization billing cycle**: An organization-level rule used to slice invoicing and resource bookings inside a longer membership term.
...
- **SC-003**: 100% of non-renewing offers create no renewal work, and 100% of renewing offers use the selected membership term for renewal decisions in acceptance tests.
...
- `MembershipTerm` remains the public name for this feature; a future rename to `PurchaseTerm` is outside scope.
