...
public class RecomputeOrganizationBookingDerivedStateShould
{
    [Theory]
    [AutoFakeItEasyData]
    ...
    {
        var rawWorkflowId = $"{Constants.RecomputeOrganizationBookingDerivedStatePrefix}-{organizationId}";

        A.CallTo(() => temporalHelperService.ToId(rawWorkflowId)).Returns(expectedWorkflowId);

        sut.RecomputeOrganizationBookingDerivedState(organizationId).ShouldBe(expectedWorkflowId);
    }
}
