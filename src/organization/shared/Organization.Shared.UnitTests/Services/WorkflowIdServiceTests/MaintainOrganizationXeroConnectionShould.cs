...
public class MaintainOrganizationXeroConnectionShould
{
    [Theory]
    [AutoFakeItEasyData]
    ...
    {
        var rawWorkflowId = $"{Constants.MaintainOrganizationXeroConnectionPrefix}-{organizationId}";

        A.CallTo(() => temporalHelperService.ToId(rawWorkflowId)).Returns(expectedWorkflowId);

        sut.MaintainOrganizationXeroConnection(organizationId).ShouldBe(expectedWorkflowId);
    }
}
