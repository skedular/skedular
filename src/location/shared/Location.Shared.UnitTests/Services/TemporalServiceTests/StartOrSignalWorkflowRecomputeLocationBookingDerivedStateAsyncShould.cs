...
public class StartOrSignalWorkflowRecomputeLocationBookingDerivedStateAsyncShould
{
    [Theory]
    [AutoFakeItEasyData]
    ...
    {
        A.CallTo(() => workflowIdService.RecomputeLocationBookingDerivedState(args.LocationId)).Returns(expectedId);
        A.CallTo(() => temporalClient.StartWorkflowAsync(
                A<Expression<Func<RecomputeLocationBookingDerivedState, Task>>>._,
        ...
        await sut.StartOrSignalWorkflowRecomputeLocationBookingDerivedStateAsync(args, cancellationToken);
