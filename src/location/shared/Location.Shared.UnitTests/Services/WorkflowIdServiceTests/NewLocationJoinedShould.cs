...
public class NewLocationJoinedShould
{
    [Theory]
    [AutoFakeItEasyData]
    ...
    {
        var rawWorkflowId = $"{Constants.NewLocationJoinedPrefix}-{locationId}";

        A.CallTo(() => temporalHelperService.ToId(rawWorkflowId)).Returns(expectedWorkflowId);

        sut.NewLocationJoined(locationId).ShouldBe(expectedWorkflowId);
    }
}
