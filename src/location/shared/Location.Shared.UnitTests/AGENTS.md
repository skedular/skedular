# Location Shared Unit Tests Agent Notes

This file applies to `location/shared/Location.Shared.UnitTests`.

## Purpose

- Unit tests for `Location.Shared` services and helpers.
...
- Group tests under the service namespace, e.g. `LocationWorkflowIdServiceTests/GenerateXShould.cs`.
...
- If the test requires real infrastructure, it belongs in `location/domain/Location.Domain.IntegrationTests` or the
