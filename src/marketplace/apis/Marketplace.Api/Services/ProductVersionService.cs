...
public class ProductVersionService(IEntityMapper entityMapper, ICachedProductVersionService cachedProductVersionService) : IProductVersionService
{
    public async Task<ProductVersion?> GetByIdAsync(string id, CancellationToken cancellationToken)
    {
        ...
        return existingProduct is null ? null : entityMapper.MapTo(existingProduct, existingProduct.Product);
    }
}
