# Team Shared Unit Tests Agent Notes

This file applies to `team/shared/Team.Shared.UnitTests`.

## Purpose

- Unit tests for `Team.Shared` services and helpers.
...
- Group tests under the service namespace, e.g. `TeamWorkflowIdServiceTests/GenerateXShould.cs`.
...
- If the test requires real infrastructure, it belongs in `team/domain/Team.Domain.IntegrationTests` or the system tests
  instead.
