...
public class EventMapper : IEventMapper
{
    ...
    private static IEnumerable<Location> MapLocations(IEnumerable<Shared.Database.Entities.Location?>? src) =>
        (src is null ? [] : src.Where(item => item is not null).Select(item => MapLocation(item, true)))!;

    private static IEnumerable<Identity> MapIdentities(IEnumerable<Shared.Database.Entities.Identity?>? src) =>
        (src is null ? [] : src.Where(item => item is not null).Select(MapIdentity))!;

    private static Identity? MapIdentity(Shared.Database.Entities.Identity? src) =>
        src is null
            ? null
            : new Identity
            {
    ...
    private static IEnumerable<Shared.Models.Resource> MapResources(IEnumerable<Resource?>? src) =>
        (src is null ? [] : src.Where(item => item is not null).Select(MapResource))!;

    private static Shared.Models.Resource? MapResource(Resource? src) =>
        src is null
            ? null
    ...
    private static IEnumerable<OrganizationTag> MapOrganizationTags(IEnumerable<Shared.Database.Entities.OrganizationTag?>? src) =>
        (src is null ? [] : src.Where(item => item is not null).Select(MapOrganizationTag))!;

    private static OrganizationTag? MapOrganizationTag(Shared.Database.Entities.OrganizationTag? src) =>
        src is null
            ? null
            : new OrganizationTag
            {
