# Customer Shared Unit Tests Agent Notes

This file applies to `customer/shared/Customer.Shared.UnitTests`.

## Purpose

- Unit tests for `Customer.Shared` services and helpers.
...
- Group tests under the service namespace, e.g. `CustomerWorkflowIdServiceTests/GenerateXShould.cs`.
...
- If the test requires real infrastructure, it belongs in `customer/domain/Customer.Domain.IntegrationTests` or the
