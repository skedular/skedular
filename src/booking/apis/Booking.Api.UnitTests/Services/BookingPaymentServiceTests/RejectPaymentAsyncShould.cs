...
public class RejectPaymentAsyncShould
{
    [Theory]
    [AutoFakeItEasyData]
        ...
        CancellationToken cancellationToken)
    {
        ...
        var result = await sut.RejectPaymentAsync(bookingId, cancellationToken);

        result.ShouldBe(mappedBooking);
        marketplaceBooking.PaymentStatus.ShouldBe(PaymentStatus.Rejected.ToPaymentStatus());
        A.CallTo(() => bookingResourceSlotsHelperService.RemoveAllSlotsFromBooking(existingBooking)).MustHaveHappenedOnceExactly();
        A.CallTo(() => temporalOutboxService.SignalWorkflowPayBookingViaCardSetPaymentStatus(
                bookingId,
                A<SetPaymentStatusArgs>.That.Matches(item => item.PaymentStatus == PaymentStatus.Rejected.ToPaymentStatus()),
                unitOfWork))
