...
const OrganizationProductsPageShell = ({ actions, toolbar, isEmpty, children }: Props) => (
  <Box sx={{ width: '100%', display: 'flex', justifyContent: 'center', px: { xs: 0, sm: 1, md: 2 }, pb: defaultPadding }}>
    <StackColumn sx={{ width: '100%', maxWidth: 1200, mx: 'auto', pt: { xs: 1, sm: 1, md: 2 } }} spacing={2}>
      <PageHeaderPanel title="Products" description="Set pricing and policies for each place. A private draft is created automatically when you add a location." />
      ...
      {isEmpty ? (
        ...
        >
          <SubtitleIconTypography label="No products yet. Add a Host location and its private draft will appear here automatically." />
        </Box>
      ) : (
        <Box
          sx={{
