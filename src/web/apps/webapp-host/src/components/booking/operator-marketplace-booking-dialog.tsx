...
import type {
  operatorMarketplaceBookingDialog_addMarketplaceBookingMutation,
  operatorMarketplaceBookingDialog_addMarketplaceBookingMutation$data,
} from '@/queries/__generated__/operatorMarketplaceBookingDialog_addMarketplaceBookingMutation.graphql';

export type OperatorMarketplaceProduct = {
  id: string;
...
type Props = {
  ...
  customerId: string;
  products: readonly OperatorMarketplaceProduct[];
  entitlements: readonly OperatorEntitlementOption[];
  onClose: () => void;
  onCompleted: () => void;
};

const OperatorMarketplaceBookingDialog = ({ open, organizationCustomDomain, customerId, products, entitlements, onClose, onCompleted }: Props) => {
    ...
    commit({
      variables: {
        input: {
          ...
          paymentMethod: entitlementId ? 'CARD' : 'CARD',
          invoiceEmailList: [],
          quantity: 1,
      ...
      onCompleted: (response: operatorMarketplaceBookingDialog_addMarketplaceBookingMutation$data) => {
        if (response.addMarketplaceBooking.booking) onCompleted();
        else setError(response.addMarketplaceBooking.accessError?.message ?? response.addMarketplaceBooking.failure?.customerAction?.name ?? 'The booking could not be created.');
      },
...
export default OperatorMarketplaceBookingDialog;
