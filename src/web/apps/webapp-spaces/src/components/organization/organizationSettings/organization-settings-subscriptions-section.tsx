...
import { SpacesQuotaStatus } from '@/components/organization/organizationSettings/organization-spaces-quota-status';
...
const offeringOrder = ['SPACES_FREE_TIER_V1', 'SPACES_GROWTH_V1', 'SPACES_BUSINESS_V1', 'SPACES_CONTACT_US_V1'];
...
const getOfferingPriceLabel = (offering: { fixedPrice: number | null | undefined; unitPrice: number | null | undefined; free: boolean; code?: string }) => {
  ...
  return ((offering.fixedPrice ?? offering.unitPrice ?? 0) / 100).toFixed(0);
};
  ...
  return (
    <>
      <Box sx={{ width: '100%' }}>
        ...
        {!organization.hasAttachedPaymentMethod && availableOfferings.some((offering) => !offering.free && !isContactUsOffering(offering.code)) && (
          <Box sx={{ pt: 2 }}>
