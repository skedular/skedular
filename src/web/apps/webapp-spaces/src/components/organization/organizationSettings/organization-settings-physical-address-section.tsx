...
import { PhysicalAddressDetails, physicalAddressSchema } from '@/components/organization/organizationSettings/organization-settings-shared';
import type { organizationSettingsPhysicalAddressSectionQuery } from '@/queries/__generated__/organizationSettingsPhysicalAddressSectionQuery.graphql';
import type { organizationSettingsPhysicalAddressSection_updateOrganizationMutation } from '@/queries/__generated__/organizationSettingsPhysicalAddressSection_updateOrganizationMutation.graphql';
...
type InnerProps = {
  organizationCustomDomain: string;
  queryReference: PreloadedQuery<organizationSettingsPhysicalAddressSectionQuery>;
  embedded?: boolean;
};
...
const RootQuery = graphql`
  query organizationSettingsPhysicalAddressSectionQuery($organizationCustomDomain: String!) {
...
const OrganizationSettingsPhysicalAddressSectionContent = ({ organizationCustomDomain, queryReference, embedded }: InnerProps) => {
  const rootData = usePreloadedQuery<organizationSettingsPhysicalAddressSectionQuery>(RootQuery, queryReference);
  const [commitUpdateOrganizationPatch] = useMutation<organizationSettingsPhysicalAddressSection_updateOrganizationMutation>(graphql`
    mutation organizationSettingsPhysicalAddressSection_updateOrganizationMutation($input: UpdateOrganizationInput!) @raw_response_type {
...
const OrganizationSettingsPhysicalAddressSection = ({ organizationCustomDomain, embedded }: Props) => {
  const [queryReference, loadQuery] = useQueryLoader<organizationSettingsPhysicalAddressSectionQuery>(RootQuery);
  ...
  return <OrganizationSettingsPhysicalAddressSectionContent organizationCustomDomain={organizationCustomDomain} queryReference={queryReference} embedded={embedded} />;
};

export default memo(OrganizationSettingsPhysicalAddressSection);
