...
locals {
  ...
  vercel_environment_variables = [
    ...
    },
    {
      ...
      value = "skedular-webapp-spaces-session-v2"
    },
    {
      key   = "WORKOS_COOKIE_DOMAIN"
      value = ".${module.shared_common.webapp_spaces_domain_name}"
    },
    {
...
resource "aws_cognito_user_pool_client" "default" {
  ...
  callback_urls = [
    ...
    "https://${module.shared_common.webapp_spaces_domain_name}/api/auth/callback/cognito"
  ]
...
resource "vercel_project" "default" {
  ...
  build_command    = "pnpm webapp-spaces#build"
  install_command  = "pnpm install --recursive --frozen-lockfile"
  output_directory = "./apps/webapp-spaces/.next"
...
resource "vercel_project_domain" "default" {
  ...
  team_id    = local.team_id
  domain     = module.shared_common.webapp_spaces_domain_name
}
...
resource "cloudflare_dns_record" "default" {
  ...
  name    = local.is_staging ? module.shared_common.webapp_spaces_domain_name : "spaces"
