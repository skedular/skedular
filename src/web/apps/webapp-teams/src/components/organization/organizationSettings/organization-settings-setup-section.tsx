...
import { getOrganizationSettingsSetupBaseLink } from '@/components/links';
import { Loading } from '@/components/loading';
import { errorNotificationOptions, NotificationContent } from '@/components/notification';
import { OrganizationMultipleChoicesIndustries } from '@/components/organization';
import { OrganizationDetails, organizationSchema, splitNotificationEmails } from '@/components/organization/organizationSettings/organization-settings-shared';
import { ImageFileUploaderWithCropper } from '@/libs/image-file-uploader';
import type { organizationSettingsSetupSectionQuery } from '@/queries/__generated__/organizationSettingsSetupSectionQuery.graphql';
import type {
  organizationSettingsSetupSection_updateOrganizationMutation,
  OrganizationBillingCycle,
  OrganizationPatchField,
} from '@/queries/__generated__/organizationSettingsSetupSection_updateOrganizationMutation.graphql';
...
type InnerProps = {
  queryReference: PreloadedQuery<organizationSettingsSetupSectionQuery>;
};
...
const RootQuery = graphql`
  query organizationSettingsSetupSectionQuery($organizationCustomDomain: String!) {
...
const OrganizationSettingsSetupSectionContent = ({ queryReference }: InnerProps) => {
  const rootData = usePreloadedQuery<organizationSettingsSetupSectionQuery>(RootQuery, queryReference);
  const [commitUpdateOrganizationPatch] = useMutation<organizationSettingsSetupSection_updateOrganizationMutation>(graphql`
    mutation organizationSettingsSetupSection_updateOrganizationMutation($input: UpdateOrganizationInput!) {
      ...
      commitUpdateOrganizationPatch({
        ...
        onCompleted: (response, errors) => {
          ...
          if (fieldsToUpdate.includes('CUSTOM_DOMAIN') && updatedCustomDomain && updatedCustomDomain !== organization.customDomain) {
            router.replace(getOrganizationSettingsSetupBaseLink(integratedPlatform, updatedCustomDomain));
          }
        },
        onError: (error) => {
          submittedPatchValues.current = previousPatchValues;
...
const OrganizationSettingsSetupSection = ({ organizationCustomDomain }: Props) => {
  const [queryReference, loadQuery] = useQueryLoader<organizationSettingsSetupSectionQuery>(RootQuery);
  ...
  return <OrganizationSettingsSetupSectionContent queryReference={queryReference} />;
};

export default memo(OrganizationSettingsSetupSection);
