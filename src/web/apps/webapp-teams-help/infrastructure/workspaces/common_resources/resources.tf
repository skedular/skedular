...
resource "vercel_project" "default" {
  ...
  build_command    = "pnpm webapp-teams-help#build"
  install_command  = "pnpm install --recursive --frozen-lockfile"
  output_directory = "./apps/webapp-teams-help/.next"
...
resource "vercel_project_domain" "default" {
  ...
  team_id    = local.team_id
  domain     = module.shared_common.webapp_teams_help_domain_name
}
...
resource "cloudflare_dns_record" "default" {
  ...
  name    = local.is_staging ? module.shared_common.webapp_teams_help_domain_name : "help.teams"
