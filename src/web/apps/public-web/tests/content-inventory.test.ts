...
import {
  capabilityClaims,
  competitorClaimReview,
} from "../src/data/claim-review";
...
describe("public website content inventories", () => {
  it("tracks all current public resource content with destinations or redirects", () => {
    for (const article of resourceArticles) {
      expect(article.sourceUrl).toMatch(/^https:\/\/getskedular\.com/);
      expect(article.destinationPath).toBeTruthy();
  ...
  it("covers every major draft heading and routes future items out of current-state copy", () => {
    ...
    expect(
      draftCoverageItems.every(
        (item) => item.heading && item.destinationRef && item.decision,
      ),
    ).toBe(true);
    expect(
      futureFeatures.every((item) => item.status === "future-planning"),
    ).toBe(true);
  });
  ...
  it("keeps comparison and capability claims in reviewable inventories", () => {
    ...
    expect(
      comparisonPages.every(
        (page) => page.competitorReviewStatus === "pending",
      ),
    ).toBe(true);
  ...
  it("tracks migrated company and legal pages with source URLs and review status", () => {
    ...
    expect(
      legalPages.every((page) =>
        page.sourceUrl.startsWith("https://getskedular.com/"),
      ),
    ).toBe(true);
    expect(legalPages.every((page) => page.reviewStatus === "pending")).toBe(
      true,
    );
  });

  it("defines launch review and source audit inventories", () => {
    ...
    expect(
      Object.values(launchReviewChecklist).every(
