...
- Those replicas are used to decide which Slack workspace/channel receives organization, location, or team updates,
  including daily attendance-style posts.
- `slack/` also relies on replicated auth-related state where needed; do not remove those replicas unless the routing
