# Slack Shared Unit Tests Agent Notes

This file applies to `slack/shared/Slack.Shared.UnitTests`.

## Purpose

- Unit tests for `Slack.Shared` services and helpers.
...
- Group tests under the service namespace, e.g. `SlackWorkflowIdServiceTests/GenerateXShould.cs`.
...
- If the test requires real infrastructure, it belongs in `slack/domain/Slack.Domain.IntegrationTests` or the system
  tests instead.
