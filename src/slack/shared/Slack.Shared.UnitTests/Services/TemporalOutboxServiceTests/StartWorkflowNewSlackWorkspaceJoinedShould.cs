...
public class StartWorkflowNewSlackWorkspaceJoinedShould
{
    [Theory]
    [AutoFakeItEasyData]
    public void Enqueue_Workflow_With_Correct_Options(
        ...
        NewSlackWorkspaceJoinedInput args,
        IUnitOfWork unitOfWork,
        string expectedId)
    {
        A.CallTo(() => workflowIdService.NewSlackWorkspaceJoined(args.WorkspaceId)).Returns(expectedId);

        sut.StartWorkflowNewSlackWorkspaceJoined(args, unitOfWork);

        A.CallTo(() => temporalOutboxWorkflowExecutor.Execute<NewSlackWorkspaceJoined, NewSlackWorkspaceJoinedInput>(
            args,
