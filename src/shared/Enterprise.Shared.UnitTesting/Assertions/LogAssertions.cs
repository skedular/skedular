...
namespace Enterprise.Shared.UnitTesting.Assertions;
...
public static class LogAssertions
{
    public static IAnyCallConfigurationWithNoReturnTypeSpecified ACallToLog(ILogger logger, LogLevel level) =>
        A.CallTo(logger).Where(call => call.Method.Name == "Log" && call.GetArgument<LogLevel>(0) == level);
    ...
    private static IAnyCallConfigurationWithNoReturnTypeSpecified Containing(
        ...
        string part) =>
        logCall.Where(call => call.GetArgument<IReadOnlyList<KeyValuePair<string, object>>>(2)!.ToString()!.Contains(part));

    public static IAnyCallConfigurationWithNoReturnTypeSpecified ACallToLogInfoContaining(ILogger logger, string part) =>
        ACallToLogInfo(logger).Containing(part);

    public static IAnyCallConfigurationWithNoReturnTypeSpecified ACallToLogErrorContaining(ILogger logger, string part) =>
