...
namespace Enterprise.Shared.IntegrationTesting.Aspire;
    ...
    extension(HttpClient httpClient)
    {
        public async Task WaitForReadinessAsync(CancellationToken cancellationToken) =>
            await HttpClient.WaitUntilAsync(
                async token =>
                {
                    using var response = await httpClient.GetAsync(HealthCheck.Constants.ReadinessPath, token);
                    return response.IsSuccessStatusCode;
                },
                $"Resource did not become ready at '{HealthCheck.Constants.ReadinessPath}' within {RetryCount * s_retryDelay.TotalMilliseconds}ms",
                cancellationToken);
