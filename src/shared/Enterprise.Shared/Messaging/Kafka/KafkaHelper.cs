...
{
    Task CreateTopicForEventAsync<TEvent>() where TEvent : IKafkaEvent, new();
    Task RegisterKeyProtobufSchemaAsync<TEvent>() where TEvent : class, IKafkaEvent, IMessage<TEvent>, new();
    Task RegisterValueProtobufSchemaAsync<TEvent>() where TEvent : class, IKafkaEvent, IMessage<TEvent>, new();
}
...
public class KafkaHelper : IKafkaHelper
{
    ...
    public async Task CreateTopicForEventAsync<TEvent>() where TEvent : IKafkaEvent, new()
    {
        ...
        if (allTopicsToCreate.Count != 0)
        {
            _logger.LogInformation(
                "Creating {NewTopicCount} new Kafka topic(s). EventType={EventType}",
                allTopicsToCreate.Count,
        ...
        if (allTopicsToUpdate.Count != 0)
        {
            ...
            {
                _logger.LogInformation(
                    "Increasing partitions for {UpdateTopicCount} existing Kafka topic(s). EventType={EventType}",
                    newPartitionsSpecification.Count,
                    typeof(TEvent).FullName);
    ...
    public async Task RegisterKeyProtobufSchemaAsync<TEvent>() where TEvent : class, IKafkaEvent, IMessage<TEvent>, new()
    {
    ...
    public async Task RegisterValueProtobufSchemaAsync<TEvent>() where TEvent : class, IKafkaEvent, IMessage<TEvent>, new()
    {
        if (!_kafkaConfiguration.UseSchemaRegistry)
        {
            _logger.LogDebug(
                "Schema registry is disabled; skipping value schema registration. EventType={EventType}",
                typeof(TEvent).FullName);
    ...
    private async Task RegisterRuntimeSchemaAsync<TEvent>(MessageComponentType componentType, string topic)
        where TEvent : class, IKafkaEvent, IMessage<TEvent>, new()
    {
    ...
    private (string Topic, string[] RetryTopics, string DeadLetterTopic) GetTopicNames<TEvent>() where TEvent : IKafkaEvent, new()
    {
        var @event = new TEvent();
        var topic = @event.GetTopicName(_kafkaConfiguration.OutgoingTopicPrefix);
        var retryTopics = Enumerable.Range(0, @event.GetRetryTopicCount())
