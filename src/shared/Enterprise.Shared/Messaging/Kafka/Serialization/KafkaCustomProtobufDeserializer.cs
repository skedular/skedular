...
namespace Enterprise.Shared.Messaging.Kafka.Serialization;

public class KafkaCustomProtobufDeserializer<T> : IDeserializer<T> where T : class, IMessage<T>, new()
{
    private readonly ILogger<KafkaCustomProtobufDeserializer<T>> _logger;
    private readonly MessageParser<T> _parser = new(() => new T());
    private readonly ProtobufDeserializer<T>? _schemaRegistryDeserializer;

    public KafkaCustomProtobufDeserializer(
        KafkaConfiguration kafkaConfiguration,
        ILogger<KafkaCustomProtobufDeserializer<T>> logger,
    ...
    public T Deserialize(ReadOnlySpan<byte> bytes, bool isNull, SerializationContext context)
    {
        if (isNull)
        {
            _logger.LogWarning("Received null Kafka payload during deserialization. MessageType={MessageType}, Topic={Topic}",
                typeof(T).FullName, context.Topic);
            throw new InvalidDataException("Expecting data not to be null");
        }

        var data = bytes.ToArray();
        _logger.LogDebug("Deserializing Kafka message. MessageType={MessageType}, Topic={Topic}, PayloadLength={PayloadLength}",
...
#pragma warning restore VSTHRD002
                _logger.LogDebug("Deserialized Kafka message via schema registry. MessageType={MessageType}", typeof(T).FullName);
                return result;
            }
            catch (SchemaRegistryException) when (HasLegacyEmptySchemaHeader(data))
            {
                _logger.LogDebug("Schema registry deserialization failed; falling back to legacy payload parsing. MessageType={MessageType}",
                    typeof(T).FullName);
