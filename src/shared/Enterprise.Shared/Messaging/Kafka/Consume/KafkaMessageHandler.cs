...
public interface IKafkaMessageHandler<TKey, TEvent> where TKey : new() where TEvent : new()
{
    ...
    where TKey : new() where TEvent : new()
{
    public async Task HandleMessageAsync(ConsumeResult<byte[], byte[]> consumeResult, CancellationToken cancellationToken)
    {
        ...
        using (activitySource.StartActivity($"handler {className}"))
        {
            ...
            var eventContext = new EventContext(new KafkaEventContext(consumeResult), null);
