...
public class KafkaConsumeService<TKey, TEvent> : BackgroundService where TKey : new() where TEvent : new()
{
        ...
        TimeProvider timeProvider)
    {
        ...
        _producer = kafkaProducerFactory is null || retryTopicKafkaConfiguration is null
            ? null
            : kafkaProducerFactory.Build<byte[], byte[]>(retryTopicKafkaConfiguration);
        _consumer = kafkaConsumerFactory.Build<byte[], byte[]>(
        ...
        try
        {
            await Task.Run(async () =>
            {
                ...
                catch (Exception ex)
                {
                    ...
                    Environment.ExitCode = MessagingExitCodes.FailedToSubscribe;
                ...
                finally
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        Environment.ExitCode =
                            MessagingExitCodes.FailedToCloseConsumer;
                        _hostApplicationLifetimeWrapper.StopApplication();
                    }
                }
            }, cancellationToken);
    ...
    private async Task PauseIfNeededAsync(ConsumeResult<byte[], byte[]> consumeResult, CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogTrace("No delay - Processing message from {Topic} - {Partition}", topicPartition.Topic, topicPartition.Partition.Value);
        }
    }
