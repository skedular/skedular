...
Every event type must carry `[KafkaTopic(...)]` (see `src/shared/Api.Shared/Events/Kafka/KafkaTopicAttribute.cs`). Every
subscriber implements `IKafkaEventSubscriber<TKey, TEvent>`.
...
| `IKafkaEventSubscriber<TKey, TEvent>`      | Consumer handler interface                    |
| `IKafkaConsumerFactory`                    | Creates raw `IConsumer<TKey, TValue>`         |
| `IKafkaProducerFactory`                    | Creates raw `IProducer<TKey, TValue>`         |
...
- Use `IKafkaPushToTopic<TKey>` for higher-level push abstractions inside domain code.
