...
using Enterprise.Shared.Messaging.Kafka.Logger;
using Enterprise.Shared.Messaging.Kafka.Produce;
using Enterprise.Shared.Messaging.Kafka.Serialization;
...
using TelemetryKeys = Enterprise.Shared.Messaging.Kafka.Telemetry.TelemetryKeys;
    ...
    extension(IServiceCollection services)
    {
        ...
        public KafkaConfiguration AddKafkaWithConnectionString(IConfiguration configuration, string? connectionString, bool useTelemetry = true)
        {
            ...
            services
                ...
                .AddSingleton(typeof(ISerializer<>), typeof(KafkaCustomProtobufSerializer<>))
                .AddSingleton(typeof(IDeserializer<>), typeof(KafkaCustomProtobufDeserializer<>))
                .AddSingleton<IKafkaConsumerFactory, KafkaKafkaConsumerFactory>()
                .AddTransient<IKafkaProducerFactory, KafkaKafkaProducerFactory>()
                .AddTransient(typeof(IProducer<,>), typeof(KafkaProducerInstanceFromFactoryAdapter<,>))
                .AddSingleton<IKafkaClientNaming, KafkaClientNaming>()
                .AddSingleton(typeof(IKafkaPushToTopic<>), typeof(KafkaKafkaPushToTopic<>))
                .AddSingleton<IKafkaActivityTracer, KafkaActivityTracer>()
                .AddSingleton<IKafkaActivityStarter, KafkaActivityStarter>()
                .AddSingleton<IPropagatorFunctionProvider<Headers>, KafkaHeaderPropagatorFunctions>()
        ...
        public IServiceCollection AddKafkaEventConsumers<TSubscriber, TKey, TEvent>(KafkaConfiguration kafkaConfiguration)
            ...
            where TKey : IKafkaEvent, new()
            where TEvent : IKafkaEvent, new()
        {
            var topicSetting =
                new KafkaTopicSetting<TEvent>(kafkaConfiguration.RetryTopicCount, DelayBaseSeconds, kafkaConfiguration.IncomingTopicPrefix);
            ...
            where TEvent : IKafkaEvent, new()
        {
            ...
            var kafkaTopicInfo = KafkaTopicAttributeHelper.GetKafkaTopicInfo<TEvent>();
            var topicSetting =
                new KafkaTopicSetting<TEvent>(kafkaTopicInfo.RetryTopicCount, DelayBaseSeconds, consumerKafkaConfiguration.IncomingTopicPrefix);
            var retryTopicSetting = new KafkaTopicSetting<TEvent>(
            ...
            services.AddSingleton<IHostedService, KafkaConsumeService<TKey, TEvent>>(sp => new KafkaConsumeService<TKey, TEvent>(
                ...
                sp.GetRequiredService<IKafkaProducerFactory>(),
                sp.GetRequiredService<IHostApplicationLifetimeWrapper>(),
                sp.GetRequiredService<IKafkaMessageHandler<TKey, TEvent>>(),
                sp.GetRequiredService<IKafkaActivityTracer>(),
                sp.GetRequiredService<TimeProvider>()));
